...
void poll_connections(
    ...
    { //wait (until timeout) for sockets' data to become available:
        ...
        int ret = select(max + 1, &read_fds, &write_fds, nullptr, &tv);
    ...
    if (listen_socket != InvalidSocket && FD_ISSET(listen_socket, &read_fds)) {
        Socket got = accept(listen_socket, nullptr, nullptr);
        ...
        while (true) { //read until more data left to read
            ...
            if (ret < 0 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
                ...
                if (ret == 0) {
                    ...
                    std::cerr << "[" << where << "] recv() returned strange number of bytes, disconnecting."
                              << std::endl;
                }
                c.close();
...
#else
        ssize_t ret = send(c.socket, reinterpret_cast< char const * >(c.send_buffer.data()), c.send_buffer.size(),
                           MSG_DONTWAIT);
#endif
        if (ret < 0 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
...
Server::Server(std::string const &port) {
    ...
    { //use getaddrinfo to look up how to bind to port:
        ...
        int addrinfo_ret = getaddrinfo(nullptr, port.c_str(), &hints, &res);
        if (addrinfo_ret != 0) {
        ...
        for (struct addrinfo *info = res; info != nullptr; info = info->ai_next) {
            { //DEBUG: dump info about this address:
                ...
                if (info->ai_family == AF_INET) {
                    auto *s = reinterpret_cast< struct sockaddr_in * >(info->ai_addr);
                    inet_ntop(res->ai_family, &s->sin_addr, ip, sizeof(ip));
                    std::cout << ip << ":" << ntohs(s->sin_port);
                } else if (info->ai_family == AF_INET6) {
                    auto *s = reinterpret_cast< struct sockaddr_in6 * >(info->ai_addr);
